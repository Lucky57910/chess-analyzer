...
import { PIECE_LABEL } from "../data/insights.js";
 ...
 */
export function archiveFacts({ stats, insights, themes }) {
  const facts = [];
 ...
 */
export function buildReview({ stats, insights, themes, kind = "rated", days = null }) {
  const facts = archiveFacts({ stats, insights, themes });
  const header = [
    "Bilan d’archive.",
